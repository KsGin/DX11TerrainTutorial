...
bool SkyPlaneClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, CHAR* cloudTextureFilename, CHAR* pertrubTextureFilename)
{
	...
	skyPlaneBottom = 0.0f;
	textureRepeat = 4;
	// Set the brightness and scale and translation of the clouds.
	m_scale = 0.3f;
	m_brightness = 0.65f;
	m_translation = 0.0f;
	...
	result = LoadTextures(device, deviceContext, cloudTextureFilename, pertrubTextureFilename);
	if (!result) {
		return false;
	}
