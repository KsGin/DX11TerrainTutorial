...
bool SkyPlaneShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
	...
	CHAR skyplane_vs[] = "./source/skyplane.vs";
	CHAR skyplane_ps[] = "./source/skyplane.ps";

	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, skyplane_vs, skyplane_ps);
	if (!result) {
		return false;
	}
...
bool SkyPlaneShaderClass::Render(
	ID3D11DeviceContext *deviceContext, int indexCount, XMMATRIX worldMatrix,
	...
	result = SetShaderParameters(
		deviceContext, worldMatrix, viewMatrix, projectionMatrix, cloudTexture,
		perturbTexture, translation, scale, brightness);
	if (!result) {
		return false;
	}
...
bool SkyPlaneShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, CHAR* vsFilename, CHAR* psFilename)
{
	HRESULT result;
...
void SkyPlaneShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, CHAR* shaderFilename)
{
	char* compileErrors;
