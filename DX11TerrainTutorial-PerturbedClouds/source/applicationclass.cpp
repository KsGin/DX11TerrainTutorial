...
bool ApplicationClass::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	// Load texture 0
	CHAR dirt01d[] = "./data/dirt01d.tga";
	result = m_TextureManager->LoadTexture(m_Direct3D->GetDevice(), m_Direct3D->GetDeviceContext(), dirt01d , 0);
	if (!result) {
		return false;
	}

	// Load texture 1
	CHAR dirt01n[] = "./data/dirt01n.tga";
	result = m_TextureManager->LoadTexture(m_Direct3D->GetDevice(), m_Direct3D->GetDeviceContext(), dirt01n , 1);
	if (!result) {
		return false;
	}
